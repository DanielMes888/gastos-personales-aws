...
export function PresupuestoMensual({ expenses, onLoadBudget, onUpdateBudget }) {
  ...
  return (
    <><header className="page-header"><div><p className="eyebrow">PLANIFICACIÓN</p><h1>Presupuesto mensual</h1><p className="page-subtitle">Define límites realistas y recibe alertas a tiempo.</p></div></header>
      <section className="budget-layout"><form className="panel budget-form" onSubmit={handleSubmit}><div><p className="eyebrow">CONFIGURACIÓN</p><h2>Tu límite del mes</h2></div>{saved && <p className="form-message success" role="status">Presupuesto actualizado correctamente.</p>}{error && <p className="form-message error" role="alert">{error}</p>}<FormField label="Mes" type="month" value={month} onChange={(event) => setMonth(event.target.value)} required /><FormField label="Límite mensual" type="number" min="1" step="0.01" value={limit} onChange={(event) => setLimit(event.target.value)} placeholder="1200.00" required /><label className="field"><span>Porcentaje de alerta: {alert}%</span><input className="range" type="range" min="50" max="95" step="5" value={alert} onChange={(event) => setAlert(event.target.value)} /><small>Te avisaremos cuando tus gastos alcancen este porcentaje.</small></label><button className="primary" type="submit" disabled={loading}>{loading ? 'Cargando…' : 'Guardar presupuesto'}</button></form>
